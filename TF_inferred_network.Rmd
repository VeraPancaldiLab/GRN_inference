...
```{r}
links_filtered_TF_2 <- read.csv("Results/links_filtered_TF_2.csv")
links_filtered_TF_2 <- links_filtered_TF_2[, -c(1)]
```
...
```{r}
g1 <- graph.data.frame(links_filtered_TF_2, directed = TRUE) #create graph from datatable
```
...
vcount(g1) #368
ecount(g1) #295
vcount(giant_comp) #140
ecount(giant_comp) #153
is.connected(giant_comp)

g = as_data_frame(giant_comp)
names(g)[names(g) == "from"] <- "Regulatory Gene"
names(g)[names(g) == "to"] <- "Target Gene"
write.csv(g,"Results/Giant_comp_0.02.csv", row.names = TRUE, quote = FALSE)
```
...
ggplot() + aes(clu1$csize) + 
  ...
  labs(title="Cluster size distribution", subtitle="Histogram of size of connected components", caption="Node list : 368 \n Edge list : 295") +
  theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"), plot.subtitle = element_text(hjust = 0.5), plot.caption = element_text(face = "italic", color = "gray40")) +
  xlab("Cluster size") + ylab("Frequency")
```
...
gg = read.csv("Results/Graph_data_info_2.csv")
```
...
On Cytoscape, add via *Import Table from File*
...
vertex.frame.color=
rainbow(9)[g1.clusters$membership],
vertex.color=
rainbow(9)[g1.clusters$membership],
vertex.label=NA, edge.color="grey")
```
