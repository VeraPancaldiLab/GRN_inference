...
all_data = lapply(list(Patient_1_rep_1_filter, Patient_1_rep_2_filter, Patient_2_rep_1_filter, Patient_2_rep_2_filter), cbind) %>% as.data.frame()
...
Patient_1_rep_1_filter = as.matrix(batch_corrected_data[,1:11])
Patient_1_rep_2_filter = as.matrix(batch_corrected_data[,12:22])
Patient_2_rep_1_filter = as.matrix(batch_corrected_data[,23:35])
Patient_2_rep_2_filter = as.matrix(batch_corrected_data[,36:48])
...
links_filtered_TF <- filter(link.list, link.list$weight >= 0.01) #3889 links
...
vcount(giant_comp) #983
ecount(giant_comp) #3888
plot(giant_comp)
...
clu1 = components(g1)
clu1$no
clu1$csize #one connected component of 983 nodes
```
...
Remove optionally zinc finger (with high gene degree) and look at the connectivity of the network.
...
g1_selected = delete_vertices(g1,c("ZNF280D","ZNF217", "ZNF318", "ZNF225")) #remove zinc fingers with the highest gene degree
g1_selected #979 nodes and 3735 edges
sort(degree(g1_selected), decreasing = TRUE)[1:20]
remove_gene = c("ZNF280D","ZNF217", "ZNF318", "ZNF225")

clug1 <- components(g1_selected)
groups(clug1)
clug1$no #single cluster for the whole network
clug1$csize #cluster of 979 already connected
...
vcount(giant_comp_selected_leaffree) #797
ecount(giant_comp_selected_leaffree) #3162
...
#Choose either link list with ZFs having a high gene degree removed or link list with all ZFs
links_filtered_TF_removed <- filter(link.list_2, link.list_2$weight >= 0.02) #280 links
links_filtered_TF_2 <- filter(link.list, link.list$weight >= 0.02) #320 links
...
nb_cluster = length(table(g1.clusters$membership)) #25 clusters
...
On Cytoscape, add via *Import Table from File*, then select *Import Data as: Node Table Columns* and identify name as *Key*
